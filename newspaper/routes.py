...
import pandas as pd
from newspaper.fetch_data import fetch_merge_analyze_data_new, \
    lengths_of_keywords
...
def index():
    ...
    mapping = pd.DataFrame({'Newspaper': [
        'The Himalayan Times',
        'Katmandu Tribune',
        'Lokaantar',
        'Nepali Sansar',
    ...
    (len_data_level_1, len_data_level_2, len_data_level_3) = \
        lengths_of_keywords()
    ...
    chart3 = [{'name': 'Level 2', 'data': df['Level2'].tolist()},
              {'name': 'Total Articles', 'data': df['News Articles'
              ].tolist()}]
    chart4 = (df['Level2'] / df['News Articles'] * 100).tolist()

    chart5 = [{'name': 'Level 3', 'data': df['Level3'].tolist()},
              {'name': 'Total Articles', 'data': df['News Articles'
              ].tolist()}]
    ...
    level2 = df[['Newspaper', 'Level2', 'Filtered Articles',
                'News Articles', 'Level2 %']]
    column_names_level2 = level2.columns.values
    row_data_level2 = list(level2.values.tolist())

    level3 = df[['Newspaper', 'Level3', 'Filtered Articles',
                'News Articles', 'Level3 %']]
...
def request_zip():
    ...
    with zipfile.ZipFile(data, mode='w') as z:
        for f_name in base_path.iterdir():
            z.write(f_name)
    data.seek(0)
    return flask.send_file(data, mimetype='application/zip',
                           as_attachment=True,
                           attachment_filename=date.today().strftime('%B_%d'
                           ) + '_datasets.zip')
