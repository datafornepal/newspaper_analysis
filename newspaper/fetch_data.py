...
def fetch_merge_analyze_data_new():
    ...
    raw_HT = pd.DataFrame(performRSSNew(himalayan_times_url)).dropna()
    ...
    raw_OK = pd.DataFrame(performRSSNew(online_khabar_url)).dropna()
    ...
    raw_NT = pd.DataFrame(performRSSNew(nepali_times_url)).dropna()
    ...
    raw_TN = pd.DataFrame(performRSSNew(telegraph_nepal_url)).dropna()
    ...
    raw_KT = pd.DataFrame(performRSSNew(kathmandu_tribune_url)).dropna()
    ...
    raw_LK = pd.DataFrame(performRSSNew(lokaantar_url)).dropna()
